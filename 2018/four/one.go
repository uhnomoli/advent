...
func PrintEntries(events []*Event) {
    ...
    sort.Slice(schedules, func(i, j int) (bool) {
        return schedules[i].total > schedules[j].total
    })

    fmt.Println()
    for _, schedule := range schedules {
        fmt.Printf("#%04d, %d minutes\n", schedule.id, schedule.total)
    }
    ...
    for _, minute := range schedule.minutes {
        fmt.Printf("00:%02d %d times\n", minute, schedule.occurrences[minute])
    }
